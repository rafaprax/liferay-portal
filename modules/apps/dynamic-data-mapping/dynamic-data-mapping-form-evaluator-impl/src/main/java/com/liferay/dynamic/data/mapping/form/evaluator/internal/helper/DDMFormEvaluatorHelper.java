...
public class DDMFormEvaluatorHelper {
	...
	protected void evaluateVisibilityExpression(Entry<String, String> entry) {
		try {
			Boolean visible = evaluateExpression(entry.getValue());
	...
	protected boolean filterVisibleFieldsMarkedAsRequired(
		...
		if (!fieldIs(ddmFormFieldContextKey, "required", false)) {
			return false;
		}

		return fieldIs(ddmFormFieldContextKey, "visible", true);
	}

	protected Map<DDMFormFieldContextKey, Map<String, Object>>
		getDDMFormFieldsPropertyChanges() {
	...
	protected Set<Integer> getDisabledPagesIndexes() {
		return ddmFormEvaluatorExpressionActionHandler.
			getDisabledPagesIndexes();
	}
	...
	protected void setRequiredErrorMessage(
		DDMFormFieldContextKey fieldContextKey) {
	...
	protected void verifyFieldsMarkedAsRequired() {
		Set<Entry<String, DDMFormField>> entrySet =
			_ddmFormFieldsMap.entrySet();

		Stream<Entry<String, DDMFormField>> stream = entrySet.stream();
