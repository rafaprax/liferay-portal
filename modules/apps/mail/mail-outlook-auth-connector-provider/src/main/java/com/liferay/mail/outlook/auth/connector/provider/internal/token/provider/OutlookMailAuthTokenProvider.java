...
import com.liferay.mail.outlook.auth.connector.provider.internal.configuration.OutlookAuthConnectorCompanyConfiguration;
...
public class OutlookMailAuthTokenProvider implements MailAuthTokenProvider {
	...
	public String getAccessToken(long companyId) {
		try {
			OutlookAuthConnectorCompanyConfiguration
				outlookAuthConnectorCompanyConfiguration =
					_configurationProvider.getCompanyConfiguration(
						OutlookAuthConnectorCompanyConfiguration.class,
						companyId);

			ConfidentialClientApplication confidentialClientApplication =
				ConfidentialClientApplication.builder(
					outlookAuthConnectorCompanyConfiguration.clientId(),
					ClientCredentialFactory.createFromSecret(
						outlookAuthConnectorCompanyConfiguration.clientSecret())
				).authority(
					String.format(
						_AUTHORITY,
						outlookAuthConnectorCompanyConfiguration.tenantId())
				).build();
