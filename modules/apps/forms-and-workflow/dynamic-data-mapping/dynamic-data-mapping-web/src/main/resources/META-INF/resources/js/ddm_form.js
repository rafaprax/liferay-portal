		...
		var LinkToPageField = A.Component.create(
			{
				...
				prototype: {
					...
					_canLoadMore: function(key, start, end) {
						...
						var cachedIndexes = instance._getCachedIndexes(key);

						return !cachedIndexes || start < cachedIndexes.start || end > cachedIndexes.end;
					},
					...
					_getCachedIndexes: function(key) {
						var instance = this;

						var indexes;

						if (instance._indexes && instance._indexes[key]) {
							indexes = instance._indexes[key];
						}

						return indexes;
					},

					_getCachedLayouts: function(key) {
						var instance = this;

						return instance._linkToPageLayoutsMap[key];
					},
					...
					_handleModalScroll: function(event) {
						...
						if (!instance._isListNodeEmpty(key)) {
							...
							var end = cachedIndexes.end;
							var start = cachedIndexes.start;
					...
					_isListNodeEmpty: function(key) {
						...
						return !instance._getCachedIndexes(key);
					},
					...
					_renderLayoutsFragment: function(layouts, key, direction) {
						...
						if (direction === 'up') {
							var cachedIndex = instance._getCachedIndexes(key);

							listNode.toggleClass('top-ended', cachedIndex.start === 0);

							for (index = total - 1; index >= 0; index--) {
								instance._addListElement(layouts[index], listNode, false, true);
							}

							if (cachedIndex.start > 0) {
					...
					_requestLayouts: function(parentLayoutId, groupId, privateLayout, start, end, callback) {
						...
						var cachedLayouts = instance._getCachedLayouts(key);

						if (start < 0 || (cachedLayouts && end > cachedLayouts.total)) {
							return;
						}
						...
						else if (cachedLayouts) {
							callback.call(instance, cachedLayouts);
						}
					},
					...
					_updateCache: function(key, layouts, start, end, total) {
						...
						var cachedLayouts = instance._linkToPageLayoutsMap[key];

						if (!instance._linkToPageLayoutsMap[key]) {
							instance._linkToPageLayoutsMap[key] = layouts;
						}

						if (!instance._indexes[key]) {
							instance._indexes[key] = {
								end: end,
								start: start,
								total: total
							};
							...
							if (instance._indexes[key].start > start) {
								cachedLayouts = layouts.concat(cachedLayouts);

								instance._indexes[key].start = start;
							}

							if (instance._indexes[key].end < end) {
								cachedLayouts = cachedLayouts.concat(layouts);

								instance._indexes[key].end = end;
							}

							instance._linkToPageLayoutsMap[key] = cachedLayouts;
						}
					}
				}
			}
