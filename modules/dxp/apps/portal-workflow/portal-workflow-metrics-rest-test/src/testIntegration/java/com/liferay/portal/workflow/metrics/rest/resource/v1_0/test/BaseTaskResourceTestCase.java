...
public abstract class BaseTaskResourceTestCase {
	...
	public void testDeleteProcessTask() throws Exception {
		...
		assertHttpResponseStatusCode(
			204,
			taskResource.deleteProcessTaskHttpResponse(null, task.getId()));

		assertHttpResponseStatusCode(
			404, taskResource.getProcessTaskHttpResponse(null, task.getId()));

		assertHttpResponseStatusCode(
			404, taskResource.getProcessTaskHttpResponse(null, 0L));
	}
	...
	@Test
	public void testPutProcessTask() throws Exception {
		Task postTask = testPutProcessTask_addTask();

		Task randomTask = randomTask();

		Task putTask = taskResource.putProcessTask(
			postTask.getProcessId(), postTask.getId(), randomTask);

		assertEquals(randomTask, putTask);
		assertValid(putTask);

		Task getTask = taskResource.getProcessTask(
			putTask.getProcessId(), putTask.getId());

		assertEquals(randomTask, getTask);
		assertValid(getTask);
	}

	protected Task testPutProcessTask_addTask() throws Exception {
		throw new UnsupportedOperationException(
			"This method needs to be implemented");
	}
	...
	protected void assertValid(Task task) {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("tokenId", additionalAssertFieldName)) {
	...
	protected boolean equals(Task task1, Task task2) {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("tokenId", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						task1.getTokenId(), task2.getTokenId())) {
	...
	protected boolean equalsJSONObject(Task task, JSONObject jsonObject) {
		for (String fieldName : getAdditionalAssertFieldNames()) {
			...
			if (Objects.equals("tokenId", fieldName)) {
				if (!Objects.deepEquals(
						task.getTokenId(), jsonObject.getLong("tokenId"))) {
	...
	protected String getFilterString(
		...
		if (entityFieldName.equals("tokenId")) {
			throw new IllegalArgumentException(
				"Invalid entity field " + entityFieldName);
		}
		...
		return new Task() {
			{
				...
				tokenId = RandomTestUtil.randomLong();
			}
		};
	}
