...
public abstract class BaseReindexStatusResourceTestCase {
	...
	protected void assertEqualsJSONArray(
		List<ReindexStatus> reindexStatuses, JSONArray jsonArray) {

		for (ReindexStatus reindexStatus : reindexStatuses) {
			boolean contains = false;

			for (Object object : jsonArray) {
				if (equalsJSONObject(reindexStatus, (JSONObject)object)) {
					contains = true;
			...
			Assert.assertTrue(
				jsonArray + " does not contain " + reindexStatus, contains);
		}
	}
	...
	protected boolean equalsJSONObject(
		ReindexStatus reindexStatus, JSONObject jsonObject) {

		for (String fieldName : getAdditionalAssertFieldNames()) {
			if (Objects.equals("completionPercentage", fieldName)) {
				if (!Objects.deepEquals(
						reindexStatus.getCompletionPercentage(),
						jsonObject.getLong("completionPercentage"))) {
			...
			if (Objects.equals("key", fieldName)) {
				if (!Objects.deepEquals(
						reindexStatus.getKey(), jsonObject.getString("key"))) {
