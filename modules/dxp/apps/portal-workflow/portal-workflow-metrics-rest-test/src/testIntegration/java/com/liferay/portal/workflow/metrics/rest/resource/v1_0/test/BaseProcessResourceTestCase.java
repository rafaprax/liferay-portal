...
public abstract class BaseProcessResourceTestCase {
	...
	public void testPutProcess() throws Exception {
		Process postProcess = testPutProcess_addProcess();

		Process randomProcess = randomProcess();

		Process putProcess = processResource.putProcess(
			postProcess.getId(), randomProcess);

		assertEquals(randomProcess, putProcess);
		assertValid(putProcess);

		Process getProcess = processResource.getProcess(putProcess.getId());

		assertEquals(randomProcess, getProcess);
		assertValid(getProcess);
	}
