...
import com.liferay.portal.workflow.metrics.rest.internal.odata.entity.v1_0.AssigneeEntityModel;
...
public class AssigneeMetricResourceImpl
	...
	private long _getAssigneesCount(
			boolean completed, Date dateEnd, Date dateStart, long processId,
			String[] taskKeys, Set<Long> userIds)
		throws Exception {
	...
	private List<AssigneeMetric> _getAssigneeUsers(
		...
		return Stream.of(
			...
			bucket -> {
				AssigneeMetric assigneeMetric = _toAssigneeMetric(bucket);

				_populateAssigneeUserWithSLAMetrics(assigneeMetric, bucket);
				_setDurationTaskAvg(assigneeMetric, bucket);
				_setTaskCount(assigneeMetric, bucket);
	...
	private void _populateAssigneeUserWithSLAMetrics(
		AssigneeMetric assigneeMetric, Bucket bucket) {

		_setOnTimeTaskCount(assigneeMetric, bucket);
		_setOverdueTaskCount(assigneeMetric, bucket);
	}

	private void _setDurationTaskAvg(
		AssigneeMetric assigneeMetric, Bucket bucket) {
	...
	private void _setOnTimeTaskCount(
		AssigneeMetric assigneeMetric, Bucket bucket) {
	...
	private void _setTaskCount(AssigneeMetric assigneeMetric, Bucket bucket) {
		if (bucket == null) {
			return;
		}
	...
	private static final EntityModel _entityModel = new AssigneeEntityModel();
