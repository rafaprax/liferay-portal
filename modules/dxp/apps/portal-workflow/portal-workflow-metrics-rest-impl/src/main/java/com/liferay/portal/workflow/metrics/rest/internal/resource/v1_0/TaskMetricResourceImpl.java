...
import com.liferay.portal.workflow.metrics.rest.internal.odata.entity.v1_0.TaskMetricEntityModel;
 ...
 */
@Component(
	properties = "OSGI-INF/liferay/rest/v1_0/task-metric.properties",
	scope = ServiceScope.PROTOTYPE, service = TaskMetricResource.class
)
public class TaskMetricResourceImpl
	extends BaseTaskMetricResourceImpl implements EntityModelResource {
	...
	@Override
	public Page<TaskMetric> getProcessTaskMetricsPage(
		...
		Map<String, TaskMetric> tasksMap = _getTaskMetricsMap(
	...
	private TaskMetric _createTaskMetric(String taskName) {
	...
	private Map<String, TaskMetric> _getTaskMetricsMap(
		String key, long processId, Set<String> taskNames, String version) {
		...
		return Stream.of(
			...
			this::_createTaskMetric
		).sorted(
			(taskMetric1, taskMetric2) -> {
				Task task1 = taskMetric1.getTask();
				Task task2 = taskMetric2.getTask();

				String taskName1 = task1.getName();

				return taskName1.compareTo(task2.getName());
			}
		).collect(
			LinkedHashMap::new,
			(map, taskMetric) -> {
	...
	private Collection<TaskMetric> _getTasks(
		...
		return Stream.of(
			...
			bucket -> {
				TaskMetric taskMetric = tasksMap.remove(bucket.getKey());

				_populateTaskWithSLAMetrics(bucket, completed, taskMetric);
				_setDurationAvg(bucket, taskMetric);
				_setInstanceCount(bucket, taskMetric);
	...
	private void _populateTaskWithSLAMetrics(
		Bucket bucket, boolean completed, TaskMetric taskMetric) {

		if (completed) {
			_setBreachedInstanceCount(bucket, taskMetric);
			_setBreachedInstancePercentage(bucket, taskMetric);
		}
		else {
			_setOnTimeInstanceCount(bucket, taskMetric);
			_setOverdueInstanceCount(bucket, taskMetric);
		}
	}

	private void _setBreachedInstanceCount(
		Bucket bucket, TaskMetric taskMetric) {
	...
	private void _setDurationAvg(Bucket bucket, TaskMetric taskMetric) {
		if (bucket == null) {
			return;
		}
	...
	private void _setInstanceCount(Bucket bucket, TaskMetric taskMetric) {
		if (bucket == null) {
			return;
		}
	...
	private void _setOnTimeInstanceCount(Bucket bucket, TaskMetric taskMetric) {
		if (bucket == null) {
			return;
		}
	...
	private static final EntityModel _entityModel = new TaskMetricEntityModel();
