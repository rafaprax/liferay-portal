...
public class KaleoDefinitionModelListener
	...
	public void onAfterUpdate(KaleoDefinition kaleoDefinition)
		...
		_processWorkflowMetricsIndexer.updateProcess(
			Optional.of(kaleoDefinition.isActive()),
			kaleoDefinition.getCompanyId(),
			Optional.of(kaleoDefinition.getDescription()),
			kaleoDefinition.getModifiedDate(),
			kaleoDefinition.getKaleoDefinitionId(),
			Optional.of(kaleoDefinition.getTitle(defaultLanguageId)),
			Optional.of(kaleoDefinition.getTitleMap()),
			Optional.of(
				StringBundler.concat(
					kaleoDefinition.getVersion(), CharPool.PERIOD, 0)));
	}
